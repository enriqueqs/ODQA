...
# From record 10 (included) to record 20 (excluded) of `train` split.
train_0_10000_ds = load_dataset('trivia_qa','rc', split=ReadInstruction('train', from_=0, to=10000, unit='abs'))

def generateDict(data) -> dict:
    out = {}
    counter = 0
    for question in data:
        idx= counter
        q = question['question']
        qty_orig_source=len(question['search_results']['rank'])
        if (qty_orig_source < 1) :
            counter+=1
            continue
        if (len(question['answer']['aliases'])< 1) :
            counter+=1
            continue
        counter+=1
        paragraphs = td.getDocBlocks(idx)
        answer = question['answer']['value']

        out[idx]={'question': q,'paragraphs': paragraphs, 'answer': answer}

    return out


reduced_dataset = generateDict(train_0_10000_ds)
data_path="/Users/enriqueqs/Github_repos/ODQA/data/processed"
with open(data_path + '/dict_idq_paragraphs_answer.pickle', 'wb') as f:
    pickle.dump(reduced_dataset, f)